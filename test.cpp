...
double mpi(int np, int ng, int na,int * asignaturas,int niteraciones,int nparticulas,int ngeneraciones,int size,int myrank);
double secuencial(int np, int ng, int na,int * asignaturas,int niteraciones,int nparticulas);
double openmp(int np, int ng, int na,int * asignaturas,int niteraciones,int nparticulas,int nhilos);
...
int main(int argc, char *argv[]){
     if (argc < 5) {
        ...
        cout << "./test <niteraciones> <nparticulas> <nhilos> <ngeneraciones> < <fichero_caso>" <<endl;
        return -1;
     }
    ...
    cout << "Ejecucion secuencial :" <<endl;
    gettimeofday(&ti, NULL); 
    double fits = secuencial(np,ng,na,datos,niteraciones,nparticulas);
    gettimeofday(&tf, NULL);
    tiempo= (tf.tv_sec - ti.tv_sec)*1000000 + (tf.tv_usec - ti.tv_usec);
    cout << "Fit:  "<<fits<<"| Tiempo: "<<tiempo/1000000 << " segundos" <<endl;
    cout << "---------------------"<<endl;
    cout << "Ejecucion OPENMPP :" <<endl;
    gettimeofday(&ti, NULL); 
    double fito = openmp(np,ng,na,datos,niteraciones,nparticulas,nhilos);
    gettimeofday(&tf, NULL);
    tiempo= (tf.tv_sec - ti.tv_sec)*1000000 + (tf.tv_usec - ti.tv_usec);
    cout << "Fit:  "<<fito<<"| Tiempo: "<<tiempo/1000000 << " segundos" <<endl;

    cout << "---------------------"<<endl;
    cout << "Ejecucion MPI :" <<endl;
    gettimeofday(&ti, NULL); 
    int size,myrank;
    MPI_Init(&argc,&argv);
    ...
    double fitm = mpi(np,ng,na,datos,niteraciones,nparticulas,ngeneraciones,size,myrank);

    gettimeofday(&tf, NULL);
    tiempo= (tf.tv_sec - ti.tv_sec)*1000000 + (tf.tv_usec - ti.tv_usec);
    cout << "Fit:  "<<fitm<<"| Tiempo: "<<tiempo/1000000 << " segundos" <<endl;
    MPI_Finalize();
