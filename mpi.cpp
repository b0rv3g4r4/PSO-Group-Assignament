...
int main(int argc, char *argv[]){
     if (argc < 4) {
        ...
        cout << "./particulas_openmp <niteraciones> <nparticulas> <ngeneraciones> < <fichero_caso>" <<endl;
        return -1;
     }
     int niteraciones = atoi(argv[1]);
     int nparticulas = atoi(argv[2]);
     int ngeneraciones = atoi(argv[3]);
    ...
    double fit = mpi(np,ng,na,datos,niteraciones,nparticulas,ngeneraciones,size,myrank);
    printf("Fitness = %f\n",fit);
